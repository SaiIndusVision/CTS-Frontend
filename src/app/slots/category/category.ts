...
import { ApiService, Category, CategoryResponse, CategoryCreateRequest, CategoryUpdateRequest } from '../../service/api.service';
...
export class CategoryComponent implements OnInit {
  ...
  totalCount = 0;
  pageSize = 10;
  viewMode: 'grid' | 'list' = 'grid'; // Changed default to 'grid'
  loading = false;
  ...
  loadCategories(page: number, pageSize: number) {
    ...
    this.apiService.listCategories(page, pageSize, filters).subscribe({
      next: (response: any) => {
        ...
        // The API response structure has these fields at root level
        if (response) {
          this.categories = Array.isArray(response.data) ? response.data : [];
          // Use total_count from the API response (not count)
